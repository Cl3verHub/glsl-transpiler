	...
	if (this.types[leftType].length == 1 || this.types[rightType].length == 1) {
		...
		if (scalar.optimize) {
			...
			if (operator == "*" || operator == "/") {
				var opFunc = (operator == "*") ? "multScalar" : "divScalar"
				info.include = opFunc;
				if (this.types[rightType].length == 1) {
					return Descriptor(`${opFunc}(${vec}, ${scalar})`, info);
				}
				else {
					return Descriptor(`${opFunc}(${scalar}, ${vec})`, info);
				}
			}
			else {
				var calcStr = this.types[rightType].length == 1 ? calculate('_', 'this', operator) :  calculate('this', '_', operator);
				return Descriptor(
					`${vec}.map(function (_) {return ${calcStr};}, ${scalar})`,info);
			}
		}
	}
	...
	if ((/mat/.test(leftType) || /mat/.test(rightType)) && operator === '*') {
		...
		if (/vec/.test(rightType)) {
			...
			for (var i = 0; i < l; i++) {
				...
				comps.push(`dot(${mat}.swizzle([${sum.join(',')}]), ${vec})`);
			}
