...
function preprocess (what, how) {
	...
	function process (str) {
		...
		if (directive) {
			...
			// console.log(directive[0], directive.input.substr(0, 20))
		}
	...
	function processIf (str) {
		...
		var matchElse = balanced('#if', '#else', match.body)

		var body, elseBody
		if (matchElse) {
			body = matchElse.body.substr(3, matchElse.body.length - 8)
			elseBody = match.body.substr(matchElse.body.length)
			elseBody = elseBody.substr(0, elseBody.length - 6)
		}
		else {
			body = match.body.substr(3)
			body = body.substr(0, body.length - 6)
			elseBody = ""
		}

		var n = body.split("\n").length
		body += "\n".repeat(elseBody.split("\n").length - 1)
		elseBody = "\n".repeat(n - 1) + elseBody
