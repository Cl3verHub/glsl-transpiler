...
function texture (sampler, coord, lod = 0) {
	...
	if (sampler[lod].faces[face].data) {
		return sampler[lod].faces[face].data.slice(idx, idx + 4);
	}
	return sampler[lod].faces[face].slice(idx, idx + 4);
}
...
function texelFetch (sampler, coord, lod = 0) {
	var size = textureSize(sampler, lod);
	var x = min(max(0, coord[0]), size[0]);
	var y = min(max(0, coord[1]), size[1]);
	var idx = y * 4 * size[0] + x * 4;
	if (sampler[lod].faces[0].data) {
		return sampler[lod].faces[0].data.slice(idx, idx + 4);
	}
	return sampler[lod].faces[0].slice(idx, idx + 4);
}
...
function textureSize (sampler, lod = 0) {
	...
	return [sampler[lod].width, sampler[lod].height];
};
