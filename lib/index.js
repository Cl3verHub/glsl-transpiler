...
GLSL.prototype.compile = function compile (arg) {
	...
	result = this.stringifyStdlib(this.includes) + '\n' + '/'.repeat(20) + " End Includes " + '/'.repeat(20) + '\n\n' + result;
	
	var pre_functions = ""
	
	if (result.indexOf(".swizzle") >= 0) {
		pre_functions += "Array.prototype.swizzle = function(positions) { var _this = this; return positions.map(function (x, i) { return this[x]}, _this) }\n"
	}
	if (result.indexOf("._xyz") >= 0) {
		pre_functions += "Array.prototype._xyz = function() { return this.slice(0, 3) }\n"
		pre_functions += "Float32Array.prototype._xyz = function() { return this.slice(0, 3) }\n"
	}
	
	result = pre_functions + '/'.repeat(20) + " Start Includes " + '/'.repeat(20) + '\n\n' + result

	result = this.fixTabs(result)
...
GLSL.prototype.transforms = {
	...
	struct: function (node) {
		...
		this.structs[structName] = {
			name: structName,
			attributeNames: argsList,
			attributeTypes: argTypes
		};
	...
	decl: function (node) {
		...
		if (node.parent.type === 'functionargs') {
			...
			if (node && node.token && node.token.data.indexOf("out") >= 0) {
				if (result.type == "float" || result.type == "int" || result.type == "bool") {
					return Descriptor(result.toString() + `/* warning: out modifier on ${result.type} variable */`, result);
				}
			}
			return result;
		}
	...
	operator: function (node) {
		...
		if (node.data === '.') {
			...
			if (this.structs[type]) {
				var index = this.structs[type].attributeNames.indexOf(prop);
				if (index >= 0) {
					type = this.structs[type].attributeTypes[index];
				}
				else {
					console.warn("Attribute type not found");
				}
			}
	...
	assign: function (node) {
		...
		if (isSwizzle) {
			var preIdent = this.process(node.children[0].children[0]);
			var positions = this.swizzlePositions(node.children[0].children[1].data);
			var len = this.types[preIdent.type].length;
	...
	unary: function (node) {
		...
		else if (node.data === '-' && /vec/.test(str.type)) {
			return Descriptor(`neg(${str})`, {type: str.type, complexity: complexity, include: 'neg'});
		}
		else {
			return Descriptor(node.data + str, {type: str.type, complexity: complexity});
		}
...
GLSL.prototype.unswizzle = function (node) {
	...
	if (canSlice() !== false) {
		var first = canSlice();
		if (first == 0 && positions.length == 3) {
			result = Descriptor(`${ident}._xyz()`, {
			...
			result = Descriptor(`${ident}.slice(${first}, ${first + positions.length})`, {
				complexity: args.length*2,
				type: `vec${args.length}`,
				components: args,
				optimize: false
		...
		result = Descriptor(`${ident}.swizzle([${positions.join(', ')}])`, {
			complexity: args.length*2,
			type: `vec${args.length}`,
			components: args,
			optimize: false
...
GLSL.prototype.fixTabs = function (code) {
	var lines = code.split("\n")
	var tab = 0
	for(var i = 0; i < lines.length; i++ ) {
		var line = lines[i]
		var tabDif = line.split("{").length - line.split("}").length
		if ((line.match(/^\t*}/)||[]).length > 0) {
			tab = Math.max(0, tab - 1)
			tabDif = Math.max(0, tabDif + 1)
		}
		lines[i] = line.replace(/^\t*/, "\t".repeat(tab))
		tab += tabDif
	}
	return lines.join("\n")
}
	...
	for (var meth in includes) {
		...
		if (includes[meth]) {
			for (var prop in includes[meth]) {
				...
				methods.push(`${meth}.${prop} = ${this.stdlib[meth][prop].toString().replace("anonymous", "").replace("\n/**/", "")}`);
			}
		}
	}
